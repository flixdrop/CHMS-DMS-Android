...
export class ProfilePage implements OnInit {
  ...
  constructor(
    private authService: AuthService,
    private dataService: DataService, // Kept unused dependency just in case it's needed later
    private navCtrl: NavController,
    private loadingCtrl: LoadingController,
    private sanitizer: DomSanitizer,
    private fcmService: FcmService,
  ...
  async ngOnInit() {
    ...
    // FIX: Await the promise from the refactored FcmService
    this.notificationsEnabled = await this.fcmService.isNotificationEnabled();
  }
  ...
  async onClickLogout(): Promise<void> {
    try {
      ...
      // FIX: Rely on AuthService to automatically pull the correct FCM token rather than hardcoding localStorage
