...
@Component({
  ...
  templateUrl: "./dryoff.page.html",
  styleUrls: ["./dryoff.page.scss"],
})
export class DryoffPage implements OnDestroy {
  @ViewChild("searchbar", { static: false }) searchbar: IonSearchbar | any;

  authUserSub: Subscription;
  fetchDataSub: Subscription;
  dryingEventSub: Subscription;
  ...
  ngOnDestroy() {
    if (this.dryingEventSub) {
      this.dryingEventSub.unsubscribe();
    }
  ...
  //             if (farm) {
  //               this.dryingEventSub = this.dataService.userData.subscribe(
  //                 (userData) => {
  //                   this.dryingAnimals = userData["drying_offs"]
  //                     .filter((dryingAnimal) => dryingAnimal.isActive === true)
  //                     .sort(
  //                       (a: any, b: any) =>
  ...
  refreshPage() {
    this.p = 1;
    return this.loadDryoffEvents();
  }

  loadDryoffEvents(): Promise<void> {
    if (!this.userId) {
      console.error("loadDryoffEvents aborted: userId is missing");
      ...
      this.fetchDataSub = this.dataService
        ...
        .subscribe({
          next: (res: any) => {
            ...
            this.dryingAnimals = this.results;
            this.isLoading = false;
            resolve();
          },
          error: (err) => {
            console.error("Component loadDryoffEvents Error:", err);
            this.isLoading = false;
            resolve();
          },
        });
