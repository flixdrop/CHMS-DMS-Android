...
export class DetailPage implements OnInit, OnDestroy {
  ...
  // 2. Load settings (if any)
  this.loadFilterSettings(); 
  
  // 3. Now start the data stream with valid dates
  this.initDataStream();
  }
    ...
    this.subscriptions.add(
      this.queryRef.valueChanges.subscribe(({ data, loading }: any) => {
        ...
        if (data?.getAnimalDetails) {
          ...
          this.lastHealth = res.lastHealth === '' ? '0%': res.lastHealth;
  ...
  return activities
    .map((item) => {
      // Assuming item.date is 'YYYY-MM-DD' and timeIntervalUtc is 'HH:mm - HH:mm'
      const [year, month, day] = item.date.split('-').map(Number);
      const [hours, minutes] = item.timeIntervalUtc.split(' - ')[0].split(':').map(Number);

      // Create as UTC, then .getTime() or .toISOString() will be handled by Chart.js
