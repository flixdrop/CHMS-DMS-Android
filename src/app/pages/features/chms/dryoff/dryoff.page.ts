...
export class DryoffPage implements OnInit, OnDestroy {
 // UI State Signals
  results = signal<any[]>([]);
...
public tableColumns: ColumnConfig[] = [
  ...
  async loadDryoffs() {
    ...
    try {
      const res = await firstValueFrom(this.chmsService.getDryoffs({
        filter: this.filter,
  ...
  refresh() { this.p.set(1); this.loadDryoffs(); }

  onPageChange(page: number) { this.p.set(page); this.loadDryoffs(); }
